...
Ext.ux.tree.TreeGridSorter = Ext.extend(Ext.tree.TreeSorter, {
    ...
    _sortFn : function (n1, n2) {
        var dsc = this.dir && this.dir.toLowerCase() == 'desc',
            p = this.property || 'text',
            sortType = this.sortType,
            fs = this.folderSort,
            cs = this.caseSensitive === true,
            leafAttr = this.leafAttr || 'leaf',
            attr1 = n1.attributes[p],
            attr2 = n2.attributes[p];
        ...
        var v1 = sortType ? sortType(attr1, n1) : (cs ? attr1 : (attr1.toUpperCase ? attr1.toUpperCase() : attr1));
        var v2 = sortType ? sortType(attr2, n2) : (cs ? attr2 : (attr2.toUpperCase ? attr2.toUpperCase() : attr2));
        if (v1 < v2) {
            return dsc ? +1 : -1;
        } else if (v1 > v2) {
            return dsc ? -1 : +1;
    ...
    onAfterTreeRender : function () {
        ...
        if (hmenu) {
            ...
            if( this.property !== "text" && this.property !== this.tree.columns[0].dataIndex){
                var ind = 0;
                for(var i = 0; i < this.tree.columns.length; i++){
                    if(this.tree.columns[i].dataIndex == this.property){
                        ind = i;
                        break;
                    }
                }
                this.tree.columns[ind].dir = this.dir || "asc";
                this.updateSortIcon(ind, this.dir || "asc");
            }
            else{
                this.tree.columns[0].dir = this.dir || "asc";
                this.updateSortIcon(0, this.dir || "asc");
            }
        }
    },
    ...
    renderElements : function (n, a, targetNode, bulkRender) {
        ...
        buf = [
             '<tbody class="x-tree-node">',
                '<tr ext:tree-node-id="', n.id ,'" class="x-tree-node-el x-tree-node-leaf', a.cls ? (" " + a.cls) : "", '">',
...
Ext.ux.tree.TreeGrid = Ext.extend(Ext.net.TreePanel, {
    rootVisible : false,
    useArrows : true,
    lines : false,
    ...
    initComponent : function () {
        ...
        if (this.enableSort || (this.sorter && !this.sorter.doSort)) {
            this.treeGridSorter = new Ext.ux.tree.TreeGridSorter(this, this.sorter || this.enableSort);
            delete this.sorter;
        }
    ...
    getAutoExpandColumn : function () {
        var num = parseInt(this.autoExpandColumn);
        if (!isNaN(num)) {
            return this.columns[num];
        }
        
        for (var i = 0, colCount = this.columns.length; i<colCount; i++) {
    ...
    autoExpand : function (preventUpdate) {
        ...
        if (this.autoExpandColumn && !this.userResized) {
            var tw = this.getTotalColumnWidth();
            var aw = this.body.getWidth(true)-(Ext.num(this.scrollOffset, Ext.getScrollBarWidth()));
            
            if (tw != aw) {
                var ci = this.getAutoExpandColumn(),
                    currentWidth = ci.width || 0,
                    cw = (aw-tw)+currentWidth;
                    
                if (!this.reserveScrollOffset || this.scrollOffset === 0) {
                    cw--;
                }
                ...
                if (cw < this.autoExpandMin) {
                    cw = this.autoExpandMin;
                }
                else if (cw > this.autoExpandMax) {
                    cw = this.autoExpandMax;
                }
